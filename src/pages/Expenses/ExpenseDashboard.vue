<template>
  <div class="p-4">
    ...
    <div class="mt-6 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
      ...
      <div class="bg-white rounded-lg border border-gray-200 p-6">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            <div class="text-sm font-medium text-gray-500">{{ t`Diesen Monat` }}</div>
            <div class="text-2xl font-bold text-gray-900">{{ formatCurrency(monthlyTotal) }}</div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-lg border border-gray-200 p-6">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            <div class="text-sm font-medium text-gray-500">{{ t`Wiederkehrend` }}</div>
            <div class="text-2xl font-bold text-gray-900">{{ formatCurrency(recurringTotal) }}</div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-lg border border-gray-200 p-6">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            <div class="text-sm font-medium text-gray-500">{{ t`Einmalig` }}</div>
            <div class="text-2xl font-bold text-gray-900">{{ formatCurrency(oneTimeTotal) }}</div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-lg border border-gray-200 p-6">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            <div class="text-sm font-medium text-gray-500">{{ t`Belege` }}</div>
            <div class="text-2xl font-bold text-gray-900">{{ totalDocuments }}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- Quick Actions -->
    <div class="mt-8">
      <h3 class="text-lg font-medium text-gray-900 mb-4">{{ t`Schnellzugriff` }}</h3>
      <div class="grid gap-4 md:grid-cols-2">
        <div class="bg-white rounded-lg border border-gray-200 p-6 hover:shadow-md transition-shadow">
          <div class="flex items-center justify-between">
            <div>
              <h4 class="text-lg font-medium">{{ t`Wiederkehrende Zahlungen` }}</h4>
              <p class="text-gray-600 text-sm mt-1">{{ t`Mieten, Abos und regelmäßige Kosten verwalten` }}</p>
            </div>
            <Button
              type="primary"
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-6 hover:shadow-md transition-shadow">
          <div class="flex items-center justify-between">
            <div>
              <h4 class="text-lg font-medium">{{ t`Einmalige Ausgaben` }}</h4>
              <p class="text-gray-600 text-sm mt-1">{{ t`Einzelne Ausgaben und Belege erfassen` }}</p>
            </div>
            <Button
              type="primary"
    ...
    <!-- Recent Expenses -->
    <div class="mt-8">
      <h3 class="text-lg font-medium text-gray-900 mb-4">{{ t`Neueste Ausgaben` }}</h3>
      <div class="bg-white rounded-lg border border-gray-200">
        <div v-if="recentExpenses.length === 0" class="p-8 text-center text-gray-500">
          <feather-icon name="inbox" class="w-12 h-12 mx-auto mb-4 text-gray-300" />
        ...
        <div v-else>
          <div
            ...
            :key="expense.name"
            class="border-b border-gray-200 last:border-b-0 p-4 hover:bg-gray-50"
          >
            <div class="flex items-center justify-between">
              <div class="flex items-center">
                <div
...
async function loadDashboardData() {
  try {
    ...
    const recurringExpenses = await fyo.db.getAll('RecurringExpense', {
      fields: ['amount', 'name', 'category', 'frequency'],
      filters: { isActive: true }
    });

    recurringTotal.value = recurringExpenses.reduce((sum, exp) => sum + (exp.amount || 0), 0);
